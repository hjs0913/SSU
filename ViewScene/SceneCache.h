...
public:
    ...
    bool Initialize(const FbxMesh *pfbxMesh);

    // Update vertex positions for deformed meshes.
    void UpdateVertexPosition(const FbxMesh *pfbxMesh, const FbxVector4 * pVertices) const;
...
struct PropertyChannel
{
    ...
    GLfloat Get(const FbxTime & rfbxTime) const
    {
        if (mAnimCurve)
        {
            return mAnimCurve->Evaluate(rfbxTime);
        }
        else
        {
            return mValue;
...
public:
    ...
    bool Initialize(const FbxLight * pLight, FbxAnimLayer * pfbxAnimLayer);

    // Draw a geometry (sphere for point and directional light, cone for spot light).
    // And set light attributes.
    void SetLight(const FbxTime & rfbxTime) const;
