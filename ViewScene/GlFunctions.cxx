...
void GlDrawMarker(FbxAMatrix& rfbxmtxGlobal)
{
    glColor3f(0.0, 1.0, 1.0);
    glLineWidth(1.0);

    glPushMatrix();
    glMultMatrixd((double*) rfbxmtxGlobal);
...
void GlDrawCamera(FbxAMatrix& rfbxmtxGlobal, double pRoll)
{
    ...
    int i;
    float pfbxCamera[10][2] = {{ 0, 5.5 }, { -3, 4.5 },
    ...
    glBegin( GL_LINE_LOOP );
    {
        for (i = 0; i < 10; i++)
        {
            glVertex3f(pfbxCamera[i][0], pfbxCamera[i][1], 4.5);
        }
    }
    glEnd();
...
void GlDrawCrossHair(FbxAMatrix& rfbxmtxGlobal)
{
