...
bool VBOMesh::Initialize(const FbxMesh *pfbxMesh)
{
    if (!pfbxMesh->GetNode())
        return(false);
    ...
    if (pfbxMesh->GetElementMaterial())
    {
        ...
        if (lMaterialIndice && lMaterialMappingMode == FbxGeometryElement::eByPolygon)
        {
            ...
            if (lMaterialIndice->GetCount() == lPolygonCount)
            {
                ...
                int lOffset = 0;
                for (int nIndex = 0; nIndex < lMaterialCount; ++nIndex)
                {
                    mSubMeshes[nIndex]->IndexOffset = lOffset;
                    lOffset += mSubMeshes[nIndex]->TriangleCount * 3;
                    // This will be used as counter in the following procedures, reset to zero
                    mSubMeshes[nIndex]->TriangleCount = 0;
                }
    ...
    mHasNormal = pfbxMesh->GetElementNormalCount() > 0;
    mHasUV = pfbxMesh->GetElementUVCount() > 0;
    ...
    if (mAllByControlPoint)
    {
        ...
        for (int nIndex = 0; nIndex < lPolygonVertexCount; ++nIndex)
        {
            // Save the vertex position.
            lCurrentVertex = lControlPoints[nIndex];
            lVertices[nIndex * VERTEX_STRIDE] = static_cast<float>(lCurrentVertex[0]);
            lVertices[nIndex * VERTEX_STRIDE + 1] = static_cast<float>(lCurrentVertex[1]);
            lVertices[nIndex * VERTEX_STRIDE + 2] = static_cast<float>(lCurrentVertex[2]);
            lVertices[nIndex * VERTEX_STRIDE + 3] = 1;
            ...
            if (mHasNormal)
            {
                ...
                lNormals[nIndex * NORMAL_STRIDE] = static_cast<float>(lCurrentNormal[0]);
                lNormals[nIndex * NORMAL_STRIDE + 1] = static_cast<float>(lCurrentNormal[1]);
                lNormals[nIndex * NORMAL_STRIDE + 2] = static_cast<float>(lCurrentNormal[2]);
            }
            ...
            if (mHasUV)
            {
                ...
                lUVs[nIndex * UV_STRIDE] = static_cast<float>(lCurrentUV[0]);
                lUVs[nIndex * UV_STRIDE + 1] = static_cast<float>(lCurrentUV[1]);
            }
        }
    ...
    for (int lPolygonIndex = 0; lPolygonIndex < lPolygonCount; ++lPolygonIndex)
    {
        ...
        for (int lVerticeIndex = 0; lVerticeIndex < TRIANGLE_VERTEX_COUNT; ++lVerticeIndex)
        {
            ...
            {
                lIndices[lIndexOffset + lVerticeIndex] = static_cast<unsigned int>(nVertices);

                lCurrentVertex = lControlPoints[lControlPointIndex];
                lVertices[nVertices * VERTEX_STRIDE] = static_cast<float>(lCurrentVertex[0]);
                lVertices[nVertices * VERTEX_STRIDE + 1] = static_cast<float>(lCurrentVertex[1]);
                lVertices[nVertices * VERTEX_STRIDE + 2] = static_cast<float>(lCurrentVertex[2]);
                lVertices[nVertices * VERTEX_STRIDE + 3] = 1;

                if (mHasNormal)
                {
                    pfbxMesh->GetPolygonVertexNormal(lPolygonIndex, lVerticeIndex, lCurrentNormal);
                    lNormals[nVertices * NORMAL_STRIDE] = static_cast<float>(lCurrentNormal[0]);
                    lNormals[nVertices * NORMAL_STRIDE + 1] = static_cast<float>(lCurrentNormal[1]);
                    lNormals[nVertices * NORMAL_STRIDE + 2] = static_cast<float>(lCurrentNormal[2]);
                }
                ...
                if (mHasUV)
                {
                    ...
                    lUVs[nVertices * UV_STRIDE] = static_cast<float>(lCurrentUV[0]);
                    lUVs[nVertices * UV_STRIDE + 1] = static_cast<float>(lCurrentUV[1]);
                }
            }
            ++nVertices;
        }
...
void VBOMesh::UpdateVertexPosition(const FbxMesh *pfbxMesh, const FbxVector4 * pVertices) const
{
    ...
    if (mAllByControlPoint)
    {
        nVertices = pfbxMesh->GetControlPointsCount();
        lVertices = new float[nVertices * VERTEX_STRIDE];
        for (int nIndex = 0; nIndex < nVertices; ++nIndex)
        {
            lVertices[nIndex * VERTEX_STRIDE] = static_cast<float>(pVertices[nIndex][0]);
            lVertices[nIndex * VERTEX_STRIDE + 1] = static_cast<float>(pVertices[nIndex][1]);
            lVertices[nIndex * VERTEX_STRIDE + 2] = static_cast<float>(pVertices[nIndex][2]);
            lVertices[nIndex * VERTEX_STRIDE + 3] = 1;
        }
    }
    else
    {
        const int lPolygonCount = pfbxMesh->GetPolygonCount();
        nVertices = lPolygonCount * TRIANGLE_VERTEX_COUNT;
        lVertices = new float[nVertices * VERTEX_STRIDE];
        ...
        for (int lPolygonIndex = 0; lPolygonIndex < lPolygonCount; ++lPolygonIndex)
        {
            ...
            {
                const int lControlPointIndex = pfbxMesh->GetPolygonVertex(lPolygonIndex, lVerticeIndex);
                lVertices[nVertices * VERTEX_STRIDE] = static_cast<float>(pVertices[lControlPointIndex][0]);
                lVertices[nVertices * VERTEX_STRIDE + 1] = static_cast<float>(pVertices[lControlPointIndex][1]);
                lVertices[nVertices * VERTEX_STRIDE + 2] = static_cast<float>(pVertices[lControlPointIndex][2]);
                lVertices[nVertices * VERTEX_STRIDE + 3] = 1;
                ++nVertices;
            }
        }
    ...
    {
        glBindBuffer(GL_ARRAY_BUFFER, mVBONames[VERTEX_VBO]);
        glBufferData(GL_ARRAY_BUFFER, nVertices * VERTEX_STRIDE * sizeof(float), lVertices, GL_STATIC_DRAW);
        delete [] lVertices;
    }
}
...
void VBOMesh::Draw(int pMaterialIndex, ShadingMode pShadingMode) const
{
    ...
    {
        for (int nIndex = 0; nIndex < mSubMeshes[pMaterialIndex]->TriangleCount; ++nIndex)
        {
...
bool LightCache::Initialize(const FbxLight * pLight, FbxAnimLayer * pfbxAnimLayer)
{
	...
	if (pfbxAnimLayer)
	{
		mColorRed.mAnimCurve = lColorProperty.GetCurve(pfbxAnimLayer, FBXSDK_CURVENODE_COLOR_RED);
		mColorGreen.mAnimCurve = lColorProperty.GetCurve(pfbxAnimLayer, FBXSDK_CURVENODE_COLOR_GREEN);
		mColorBlue.mAnimCurve = lColorProperty.GetCurve(pfbxAnimLayer, FBXSDK_CURVENODE_COLOR_BLUE);
	}
    ...
    if (mType == FbxLight::eSpot)
    {
		...
		if (pfbxAnimLayer)
			mConeAngle.mAnimCurve = lConeAngleProperty.GetCurve(pfbxAnimLayer);
    }

    return(true);
}

void LightCache::SetLight(const FbxTime & rfbxTime) const
{
    const GLfloat lLightColor[4] = {mColorRed.Get(rfbxTime), mColorGreen.Get(rfbxTime), mColorBlue.Get(rfbxTime), 1.0f};
    const GLfloat lConeAngle = mConeAngle.Get(rfbxTime);
