...
ID3D12RootSignature* CScene::CreateGraphicsRootSignature(ID3D12Device* pd3dDevice)
{
	...
	D3D12_ROOT_PARAMETER pd3dRootParameters[5]; pd3dRootParameters[0].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
	...
	pd3dDevice->CreateRootSignature(0, pd3dSignatureBlob->GetBufferPointer(), pd3dSignatureBlob->GetBufferSize(), __uuidof(ID3D12RootSignature), (void
...
void CScene::AnimateObjects(float fTimeElapsed)
{
	...
	if (m_pLights)
	{
		m_pLights->m_pLights[1].m_xmf3Position = XMFLOAT3(30.0f, 30.0f, 0.0f);
...
void CScene::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	...
	UpdateShaderVariables(pd3dCommandList);//조명 리소스에 대한 상수 버퍼 뷰를 쉐이더 변수에 연결(바인딩)한다. 
...
void CScene::BuildLightsAndMaterials()
{
	...
	m_pLights->m_pLights[3].m_fPhi = (float)cos(XMConvertToRadians(90.0f)); m_pLights->m_pLights[3].m_fTheta = (float)cos(XMConvertToRadians(30.0f));
