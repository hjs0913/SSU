...
void CGameFramework::ProcessInput()   //여기
{
	...
	if (!bProcessedByScene)
	{
		...
		if (GetAsyncKeyState('I') & 0x8000) { //방이동 
			if (pushI) {
				if (IsIn) {
		...
		if (pKeysBuffer[VK_SHIFT] & 0xF0) {  //카메라 줌 뒤로 
			cameradis = 1.05f;
		}
		else
			cameradis = 1.0f;
		...
		if (GetAsyncKeyState(VK_CONTROL) & 0x8000) {  //ctrl 키 총알 발사 부딪히면 불꽃
			if (pushCTRL) {
				++bulletidx;
				pushCTRL = false;
			}
