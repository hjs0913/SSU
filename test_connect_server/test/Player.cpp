...
CAirplanePlayer* pOtherPlayer = NULL;
...
void CAirplanePlayer::Animate_Own_Height(CGameTimer pTimer, CCamera* pCamera, CGameObject* otherplayer)
{
	CHeightMapTerrain* pTerrain = (CHeightMapTerrain*)m_pPlayerUpdatedContext;
	float fheigth = pTerrain->GetHeight(otherplayer->GetPosition().x, otherplayer->GetPosition().z);
	SetPosition(XMFLOAT3(otherplayer->GetPosition().x, fheigth+otherplayer->GetPosition().y, otherplayer->GetPosition().z));
	SetLook(otherplayer->GetLook());
}
...
CTerrainPlayer::CTerrainPlayer(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, void* pContext, int nMeshes) : CPlayer(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, pContext, nMeshes)
{
	...
	CHeightMapTerrain* pTerrain = (CHeightMapTerrain*)pContext;
	SetPosition(XMFLOAT3(pTerrain->GetWidth() * 0.5f, 20.0f, pTerrain->GetLength() * 0.5f));
	SetPlayerUpdatedContext(pTerrain);
