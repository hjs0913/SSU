...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
Texture2D gtxtTextures: register(t0);
Texture2D gtxtTextures2: register(t1);
...
float4 PSTextured(VS_TEXTURED_OUTPUT input, uint nPrimitiveID : SV_PrimitiveID) : SV_TARGET
{
	...
	float4 cColor = gtxtTextures.Sample(gWrapSamplerState, input.uv);// *gtxtTextures2.Sample(gWrapSamplerState, input.uv);
	clip(cColor.a - 0.1f);
	clip(cColor.r + cColor.g + cColor.b - 0.000001f);
	cColor.a = cColor.a - 0.7;
	return(cColor);
}
...
{
	float4 cBaseTexColor = gtxtTerrainBaseTexture.Sample(gWrapSamplerState, input.uv0) * gtxtTerrainDetailTexture.Sample(gWrapSamplerState, input.uv0);
...
float4 PSTerrainTessellation(DS_TERRAIN_TESSELLATION_OUTPUT input) : SV_TARGET
{
	...
	if (cameraposition.y >250) {
		...
		float4 cDetailTexColor = gtxtTerrainDetailTexture.Sample(gWrapSamplerState, input.uv0);

		cColor = cBaseTexColor * cDetailTexColor;
	}
	return(cColor);
}
/////////////////////////////
Texture2D refelcttex: register(t15);

VS_TEXTURED_OUTPUT VSReflect(VS_TEXTURED_INPUT input)
{
	VS_TEXTURED_OUTPUT output;
...
float4 PSReflect(VS_TEXTURED_OUTPUT input) : SV_TARGET
{
		float4 cColor = refelcttex.Sample(gWrapSamplerState, input.uv);// *gtxtTextures2.Sample(gWrapSamplerState, input.uv);
	clip(cColor.a - 0.1f);
	clip(cColor.r + cColor.g + cColor.b - 0.000001f);
	return(cColor);
}
//float4 PSReflect(VS_DIFFUSED_OUTPUT input) : SV_TARGET
//{
//	return(input.color);
//}