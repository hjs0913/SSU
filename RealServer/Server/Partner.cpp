...
void Partner::join_partner(Partner* pa)
{
	partner_party[player_cnt] = pa;

	partner_party_id[player_cnt] = pa->partner_get_id(); //

	player_cnt++;

	cout << "ai가 " << dungeon_id << "번 던전에 입장 중입니다" << endl;
	//pa->state_lock.lock();
	//pa->join_dungeon_room = true;
...
void Partner::partner_attack()  //일반공격 기본, 스킬을 쿨타임 돌때마다 계속 쓰도록 하자 
{
	...
	case 1: {
		...
		Coord a = { partners[0]->get_x(), partners[0]->get_z() };    //플레이어 기준 전방 삼각형 범위 
