...
CHeightMapGridMesh::CHeightMapGridMesh(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, int xStart, int zStart, int nWidth, int nLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color, void *pContext) : CMesh(pd3dDevice, pd3dCommandList)
{
	...
	m_pxmf2TextureCoords2 = new XMFLOAT2[m_nVertices];
	...
	for (int i = 0, z = zStart; z < (zStart + nLength); z++)
	{
		for (int x = xStart; x < (xStart + nWidth); x++, i++)
		{
			...
			m_pxmf2TextureCoords2[i] = XMFLOAT2(float(x) / float(cxHeightMap - 1), float(czHeightMap - 1 - z) / float(czHeightMap - 1));
	...
	m_pd3dTextureCoord2Buffer = ::CreateBufferResource(pd3dDevice, pd3dCommandList, m_pxmf2TextureCoords2, sizeof(XMFLOAT2) * m_nVertices, D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, &m_pd3dTextureCoord2UploadBuffer);

	m_d3dTextureCoord2BufferView.BufferLocation = m_pd3dTextureCoord1Buffer->GetGPUVirtualAddress();
	m_d3dTextureCoord2BufferView.StrideInBytes = sizeof(XMFLOAT2);
	m_d3dTextureCoord2BufferView.SizeInBytes = sizeof(XMFLOAT2) * m_nVertices;
...
CHeightMapGridMesh::~CHeightMapGridMesh()
{
	...
	if (m_pd3dTextureCoord2Buffer) m_pd3dTextureCoord2Buffer->Release();

	if (m_pxmf4Colors) delete[] m_pxmf4Colors;
	if (m_pxmf2TextureCoords0) delete[] m_pxmf2TextureCoords0;
	if (m_pxmf2TextureCoords1) delete[] m_pxmf2TextureCoords1;
	if (m_pxmf2TextureCoords2) delete[] m_pxmf2TextureCoords2;
}
...
void CHeightMapGridMesh::ReleaseUploadBuffers()
{
	...
	if (m_pd3dTextureCoord2UploadBuffer) m_pd3dTextureCoord2UploadBuffer->Release();
	m_pd3dTextureCoord2UploadBuffer = NULL;
}
...
{
	D3D12_VERTEX_BUFFER_VIEW pVertexBufferViews[5] = { m_d3dPositionBufferView, m_d3dColorBufferView, m_d3dTextureCoord0BufferView, m_d3dTextureCoord1BufferView, m_d3dTextureCoord2BufferView };
