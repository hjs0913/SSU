...
void send_skill_packet(int cl)
{
	cout << "받아라 !" << endl;
	cs_packet_skill my_packet;
	my_packet.size = sizeof(my_packet);
	my_packet.type = CS_PACKET_SKILL;
	my_packet.skill_type = 0;
	my_packet.skill_num = 0;
	SendPacket(cl, &my_packet);
...
void Adjust_Number_Of_Client()
{
	...
	if (active_clients % 5 == 0) {;
		send_skill_packet(num_connections);
	}
